...
async function testHeadlineFiltering() {
  ...
  // Valid headline
  assert(
    passesFilters('Federal Reserve Signals Potential Rate Cut in March Meeting') === true,
    'Valid headline should pass'
  );
  ...
  assert(
    passesFilters('Enable javascript to view content') === false,
    'Exclude pattern should fail'
  );
...
async function testSimilarityDetection() {
  ...
  assert(
    simDifferent < 0.2,
    `Different headlines should have similarity < 0.2, got ${simDifferent}`
  );
...
async function testConfigLoading() {
  ...
  console.log(
    `  ✓ Config loading works (${config.sources.length} sources, ${enabledSources.length} enabled)`
  );
}
